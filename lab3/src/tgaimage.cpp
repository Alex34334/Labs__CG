...
bool TGAImage::read_tga_file(const char* filename) {
    ...
    bytespp = header.bitsperpixel >> 3; // Переводим биты в байты
...
bool TGAImage::load_rle_data(std::ifstream& in) {
    ...
    do {
        ...
        if (chunkheader < 128) {
            // Raw пакет - читаем пиксели напрямую
            chunkheader++;
            ...
            // RLE пакет - читаем один пиксель и повторяем его
            chunkheader -= 127;
            in.read((char*)colorbuffer.bgra, bytespp);
            if (!in.good()) {
                std::cerr << "an error occured while reading the header\n";
...
bool TGAImage::write_tga_file(const char* filename, bool rle) {
    ...
    header.imagedescriptor = 0x20; // Верхний левый угол
...
bool TGAImage::unload_rle_data(std::ofstream& out) {
    ...
    while (curpix < npixels) {
        ...
        // Поиск последовательности одинаковых пикселей
...
// Масштабирование изображения (простой алгоритм)
